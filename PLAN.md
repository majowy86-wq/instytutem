...
- [ ] **Ten sam szablon strony zabiegu do powielenia na ~23 pozostałe zabiegi z menu** — `zabiegi/depilacja-laserowa-plock-lightsheer/index.html` to teraz wzorzec strukturalny (hero z przyciskami/pasek cena+raty/sticky-tabs/akordeon/benefity/cennik/promo/opinie/newsletter/stopka + scroll-reveal). **Ważne przy powielaniu** (Runda 77-78): strona zabiegu leży 2 poziomy głębiej niż root (`zabiegi/<slug>/index.html`), więc wszystkie własne zasoby (CSS/JS/obrazy/logo) i linki nawigacyjne muszą używać ścieżek względnych `../../...`, nie bezwzględnych `/...` — inaczej strona jest niewidoczna (brak stylowania) na GitHub Pages (podścieżka `/instytutem/`), mimo że lokalnie i docelowo na instytutem.pl (korzeń domeny) działałoby to poprawnie.
