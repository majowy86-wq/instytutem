...
10. **Boks/karta odsyłająca do zabiegu (strona główna, strony „Problem", cross-selling itd.) ZAWSZE używa dokładnie tego samego zdjęcia co `.treatment-hero-bg` i dokładnie tej samej nazwy co `.eyebrow` na WŁASNEJ podstronie tego zabiegu** — jedyny element, który wolno różnicować w zależności od kontekstu (jaki problem/potrzebę dany box adresuje), to opis. Przy zmianie hero zabiegu — przeszukać sitewide wszystkie boksy odsyłające do niego i zaktualizować zdjęcie wszędzie, nie tylko na jego własnej podstronie. Nie dotyczy zdjęć WEWNĄTRZ własnej podstrony zabiegu (np. banery/sekcje inne niż hero) — tylko boksów odsyłających z zewnątrz. Ustalone 2026-09-15.
